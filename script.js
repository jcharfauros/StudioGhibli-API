...
// const filmInfo = document.querySelector('.card-body'); //not using
...
function displayResults (ghibliFilms) {
    ghibliFilms.forEach((g, index) => {
        ...
        if (index < 5) {
            ...
            // filmTitle.setAttribute = ('card-title', 'card');
            ...
            cardBody.insertBefore(synopsis, filmInfoBtn);
            
        }
        
    });
    let randomFilm = ghibliFilms[Math.floor(Math.random()*ghibliFilms.length)]; 
